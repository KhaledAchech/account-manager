...
def server_connection() -> object:
    try:
        ...
        cprint(f"Error connecting to the PostgreSQL server: {e}", THEME.get("error"))
        exit(1)
...
def database_connection() -> object:
    if not database_exists():
        try:
            ...
            cprint(f"Database '{DB_PARAMS['database']}' created successfully.", THEME.get("success"))
        except psycopg2.Error as e:
    ...
    try:
        ...
        cprint(f"Connected to database '{DB_PARAMS['database']}'.", THEME.get("success"))
    except psycopg2.Error as e:
        cprint(f"Error connecting to the database: {e}", THEME.get("error"))
        exit(1)
    return connection

DB_INSTANT = database_connection()
